...
def collect_info(info_file, user, psswd, keyLocation, localSrc, remoteDst ):
    ...
    #try to copy the latest EIBL code from the execution server to each of the
    #nodes in the GENI
...
def trigger_EIBL(info_dict, user, pswd, keyLocation, cmd_file, remoteDst):

    '''
    Execute appropriate EIBL commands on each nodes
    :return:
    '''
    ...
    for i in range(0, len(nodes)):
        ...
        if  not  nodes[i][:-1] == 'ipnode': #ipnode or host or IPN
            ...
            stdin, stdout, stderr = ssh[i].exec_command('rm -rf eibllogs', timeout=30)
            showtime = strftime("%m%d%H%M%Y.%S", localtime()) #we get the current time in format MMDDhhmmYYYY.SS
      
            #check tail -f logs in the folder where you trigger EIBL
            stdin, stdout, stderr = ssh[i].exec_command('sudo pkill EIBL', timeout=30)
            stdin, stdout, stderr = ssh[i].exec_command('uptime')
            stdin, stdout, stderr = ssh[i].exec_command('cd '+ remoteDst +'/EIBL_Code/;gcc -g -o EIBL *.c -lm', timeout=30)
            time.sleep(2)
            ...
            x ='cd ' + remoteDst + '/EIBL_Code/; sudo screen -dmS ' + str(nodes[i]) + ' -L -Logfile eibllogs ' + str(Mcmd_dict[nodes[i]])
            print(x)
            stdin, stdout, stderr = ssh[i].exec_command(x)
            
            x1 = 'sudo chmod 666 ' + remoteDst + '/EIBL_Code/eibllogs'
            stdin, stdout, stderr = ssh[i].exec_command(x1)

            sys.stdout.write("\r EIBL is Running on :%s\n" % (nodes[i]))
            ssh[i].close()
...
if __name__ == "__main__":
    ...
    trigger_EIBL(info_dict, uname, password, keyLocation, cmd_file, remoteDst)
    fileop.close()
